-- ==============================================================================
-- 00016_fix_signup_subscription_conflict.sql
-- Fix UNIQUE constraint on tenant_subscriptions(tenant_id) & handle_new_user_signup trigger
-- ==============================================================================

-- 1. Ensure unique constraint on tenant_subscriptions(tenant_id)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tenant_subscriptions'::regclass AND conname = 'tenant_subscriptions_tenant_id_key'
    ) THEN
        ALTER TABLE public.tenant_subscriptions ADD CONSTRAINT tenant_subscriptions_tenant_id_key UNIQUE (tenant_id);
    END IF;
END $$;

-- 2. Update handle_new_user_signup function to safely insert tenant subscription and user with status='active'
...
BEGIN
    ...
    ELSE
        ...
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_subscriptions') THEN
            IF NOT EXISTS (SELECT 1 FROM public.tenant_subscriptions WHERE tenant_id = new_tenant_id) THEN
